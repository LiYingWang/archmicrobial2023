...
library(rstatix)
rich_pair_V4 <- pairwise_t_test(V4_alphadiv, S.obs ~ group, p.adjust.method = 'none')
rich_pair_V1 <- pairwise_t_test(V1_alphadiv, S.obs ~ group, p.adjust.method = 'none')
rich_pair_V6 <- pairwise_t_test(V6_alphadiv, S.obs ~ group, p.adjust.method = 'none')
rich_pair_ITS1 <- pairwise_t_test(ITS1_alphadiv, S.obs ~ group, p.adjust.method = 'none')

shan_pair_V4 <- pairwise_t_test(V4_alphadiv, shannon ~ group, p.adjust.method = 'none')
shan_pair_V1 <- pairwise_t_test(V1_alphadiv, shannon ~ group, p.adjust.method = 'none')
shan_pair_V6 <- pairwise_t_test(V6_alphadiv, shannon ~ group, p.adjust.method = 'none')
shan_pair_ITS1 <- pairwise_t_test(ITS1_alphadiv, shannon ~ group, p.adjust.method = 'none')

rich_pv_v4 <- format.pval(pv = rich_pair_V4$p[6], 
              digits = 2,  # number of digits, but after the 0.0
              eps = 0.001, # above the number will be replaced  by "<0.0xxx"
              nsmall = 3) # how much tails 0 to keep 

rich_pv_v1 <- format.pval(pv = rich_pair_V1$p[4], digits = 2, eps = 0.01, nsmall = 2) 
rich_pv_its1 <- format.pval(pv = rich_pair_ITS1$p[4], digits = 2, eps = 0.05, nsmall = 2)
shan_pv_v4 <- format.pval(pv = shan_pair_V4$p[2], digits = 2, eps = 0.05, nsmall = 2) 
shan_pv_v1 <- format.pval(pv = shan_pair_V1$p[2], digits = 2, eps = 0.05, nsmall = 2) 
```

## Alpha diversity analysis of bacterial and fungal communities
Illumina analysis generated a total of `r total_reads` sequences that were normalized to `r rare_reads` after rarefaction for comparison across different types of samples [@Schloss2024]. For the V4 and V6 regions, soil samples show a significant higher richness indices based on the number of observed OTUs (p `r rich_pv_v4`) compared with those of exterior pottery samples, interior pottery samples, and negative control samples. Similar patterns were observed for the V1 and ITS1 region, but the richness index differ significantly mainly between soil samples and negative control samples (p `r rich_pv_v1` and `r rich_pv_its1` respectively; Table S1). Although there is no statistically significant differences in richness indices between the two types of pottery samples and negative control, Shannon diversity indices for the V4 and V1 regions are significantly higher from exterior pottery samples than those from negative control samples (p `r shan_pv_v4` and `r shan_pv_v1` respectively; Table S2). 
