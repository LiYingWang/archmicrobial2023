...
# pairwise t-test on richness
library(rstatix)
# no correction for the Type I error rate across the pairwise tests
...
rich_pair_region <- function(df, vector) {
  df %>%
    ...
    select(region, group1, group2, n1, n2, p, p.signif)
}
...
kable(rich_pair_all , caption = "Table S1: Pairwise comparison of Richness index (the number of observed OTUs) using ANOVA")
```
...
kable(shan_pair_all  , caption = "Table S2: Pairwise comparison of Shannon diversity indices using ANOVA")
